...
class Config:
    ...
    def _load_env_vars(self) -> None:
        ...
        self.rpc_rate_limit_per_second = int(
            os.getenv("RPC_RATE_LIMIT_PER_SECOND", "10")
        )
        ...
        self.database_url = os.getenv(
            "DATABASE_URL", "sqlite:///./data/affiliate_fees.db"
        )
    ...
    def validate(self) -> None:
        ...
        if not self.alchemy_api_key and not self.infura_api_key:
            errors.append(
                "At least one RPC provider API key is required (ALCHEMY_API_KEY or INFURA_API_KEY)"
            )

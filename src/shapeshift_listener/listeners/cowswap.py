...
class CoWSwapListener(BaseListener):
    ...
    def __init__(self, config: Config):
        ...
        self.web3 = None
        # CoW Swap uses chain-specific Safe addresses for affiliate tracking
    ...
    async def process_block(self, block_number: int) -> List[dict[str, Any]]:
        ...
        try:
            ...
            for tx in block.transactions:
                # Check if transaction involves CoW Swap affiliate address
    ...
    def _parse_affiliate_event(self, tx: Any, block_number: int) -> dict[str, Any]:
        """Parse affiliate fee event from CoW Swap transaction."""
        try:
            return {
                "protocol": "cowswap",
            ...
            self.logger.error(f"Error parsing CoW Swap affiliate event: {e}", exc_info=True)
            return None
    ...
    async def initialize(self, chain: str) -> None:
        ...
        try:
            ...
            self.logger.info(f"Initialized CoW Swap listener for {chain}")

        except Exception as e:
            self.logger.error(
                f"Failed to initialize CoW Swap listener for {chain}: {e}",
                exc_info=True,
            )
            raise

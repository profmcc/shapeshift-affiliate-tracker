...
class CSVCowSwapListener:
    ...
    def get_web3_connection(self, chain_config: Dict) -> Optional[Web3]:
        ...
        rpc_providers = [
            {"name": "Alchemy", "url": self._get_alchemy_url(chain_config["chain_id"])},
            {"name": "Infura", "url": self._get_infura_url(chain_config["chain_id"])},
        ]
        ...
        for provider in rpc_providers:
            ...
            try:
                ...
                if w3.is_connected():
                    ...
                    logger.warning(
                        f"⚠️ Failed to connect to {chain_name} via {provider['name']}"
                    )

            except Exception as e:
                logger.warning(
                    f"⚠️ Error connecting to {chain_name} via {provider['name']}: {e}"
                )
                continue
    ...
    def _get_infura_url(self, chain_id: int) -> str:
        ...
        return infura_urls.get(
            chain_id, f"https://mainnet.infura.io/v3/{self.infura_api_key}"
        )

    def _get_logs_with_fallback(
        self, w3: Web3, filter_params: Dict, chain_config: Dict
    ) -> List:
        ...
        try:
            ...
            if "400" in str(e) or "rate limit" in str(e).lower():
                logger.warning(
                    f"⚠️ Primary RPC failed ({str(e)[:100]}...), trying fallback..."
                )
                ...
                try:
                    ...
                    if backup_w3.is_connected():
                        logger.info(
                            f"✅ Connected to {chain_config['name']} via Infura fallback"
                        )
    ...
    def check_affiliate_involvement(
        self, receipt: Dict, affiliate_address: str
    ) -> bool:
        """
                ...
                if log["topics"]:
                    for topic in log["topics"]:
                        ...
                        if affiliate_clean in topic_hex:
                            logger.info(
                                f"   ✅ Found ShapeShift affiliate in topic: {topic.hex()}"
                            )
                            return True
    ...
    def fetch_cowswap_events(
        self, chain_name: str, blocks_to_scan: int = 1000
    ) -> List[Dict]:
        """
        ...
        try:
            ...
            start_block = max(
                chain_config["start_block"], latest_block - blocks_to_scan
            )

            logger.info(
                f"🔍 Scanning {chain_config['name']} blocks {start_block} to {latest_block}"
            )
            ...
            while current_block <= latest_block:
                ...
                try:
                    ...
                    for log in logs:
                        ...
                        shapeshift_affiliate = self.shapeshift_affiliates.get(
                            chain_config["chain_id"]
                        )

                        # Use the improved affiliate detection
                        affiliate_involved = self.check_affiliate_involvement(
                            tx_receipt, shapeshift_affiliate
                        )
                        ...
                        if affiliate_involved:
                            ...
                            if event_data:
                                # Get token metadata
                                sell_token_meta = self.get_token_metadata(
                                    w3, event_data["sell_token"]
                                )
                                buy_token_meta = self.get_token_metadata(
                                    w3, event_data["buy_token"]
                                )
                                ...
                                logger.info(
                                    f"   ✅ Found ShapeShift affiliate CoW Swap event: {log['transactionHash'].hex()}"
                                )
                    ...
                    logger.error(
                        f"Error fetching logs for blocks {current_block}-{end_block}: {e}"
                    )
                    current_block = end_block + 1
                    continue
    ...
    def update_block_tracker(self, chain_name: str, last_block: int):
        ...
        for row in existing_data:
            if row["chain"] == chain_name:
                ...
                row["last_processed_date"] = datetime.now().strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
                row["total_blocks_processed"] = str(
                    int(row.get("total_blocks_processed", 0)) + 1000
                )
                updated = True
                break
    ...
    def enrich_csv_with_affiliate_data(self, csv_file: str = None):
        """Enrich existing CSV data with affiliate information from CoW Swap API using order IDs."""
        if csv_file is None:
            csv_file = os.path.join(self.csv_dir, "cowswap_transactions.csv")
        ...
        logger.info(f"🔍 Enriching CSV with CoW Swap API affiliate data: {csv_file}")
        ...
        logger.info(f"📊 Found {len(transactions)} transactions to enrich")
        
        # Create enriched CSV with new columns
        enriched_file = csv_file.replace('.csv', '_enriched.csv')
        enriched_headers = list(transactions[0].keys()) + ['cow_api_affiliate', 'cow_api_app_code', 'cow_api_app_data']
        
        enriched_transactions = []
        total_enriched = 0
        
        for i, tx in enumerate(transactions):
            logger.info(f"🔍 Enriching transaction {i+1}/{len(transactions)}: {tx['tx_hash'][:10]}...")
            
            # Copy existing data
            enriched_tx = tx.copy()
            
            # Initialize new fields
            enriched_tx['cow_api_affiliate'] = ''
            enriched_tx['cow_api_app_code'] = ''
            enriched_tx['cow_api_app_data'] = ''
            
            # Try to get affiliate data from CoW API if we have an order_uid
            if tx.get('order_uid') and tx['order_uid'] != '':
                try:
                    affiliate_data = self._get_cow_affiliate_data(tx['order_uid'])
                    if affiliate_data:
                        enriched_tx['cow_api_affiliate'] = affiliate_data.get('affiliate', '')
                        enriched_tx['cow_api_app_code'] = affiliate_data.get('app_code', '')
                        enriched_tx['cow_api_app_data'] = affiliate_data.get('app_data', '')
                        total_enriched += 1
                        logger.info(f"✅ Enriched order {tx['order_uid'][:10]}... with affiliate data")
                    else:
                        logger.debug(f"⚠️ No affiliate data found for order {tx['order_uid'][:10]}...")
                except Exception as e:
                    logger.warning(f"⚠️ Failed to get affiliate data for order {tx['order_uid'][:10]}...: {e}")
            else:
                logger.debug(f"⚠️ No order_uid for transaction {tx['tx_hash'][:10]}...")
        ...
        with open(enriched_file, 'w', newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=enriched_headers)
            writer.writeheader()
            writer.writerows(enriched_transactions)
        
        logger.info(f"✅ Enrichment complete! Saved {len(enriched_transactions)} transactions to {enriched_file}")
        logger.info(f"📊 Successfully enriched {total_enriched} transactions with affiliate data")
        
        return enriched_file

    def _get_cow_affiliate_data(self, order_uid: str) -> Optional[Dict]:
        """Get affiliate data from CoW Swap API using order ID."""
        try:
            # CoW Swap API endpoint for orders
            api_url = f"https://api.cow.fi/mainnet/api/v1/orders/{order_uid}"
            
            logger.debug(f"🔍 Querying CoW API: {api_url}")
            ...
            if response.status_code == 200:
                data = response.json()
                logger.debug(f"✅ CoW API response: {data}")
                ...
                # Look for affiliate information in the response
                if 'appData' in data:
                    affiliate_data['app_data'] = data['appData']
                
                if 'appCode' in data:
                    affiliate_data['app_code'] = data['appCode']
                
                # Check if this is a ShapeShift affiliate
                if 'appCode' in data and 'shapeshift' in data['appCode'].lower():
                    affiliate_data['affiliate'] = 'ShapeShift'
                elif 'appData' in data and 'shapeshift' in data['appData'].lower():
                    affiliate_data['affiliate'] = 'ShapeShift'
                
                return affiliate_data
                
            elif response.status_code == 404:
                logger.debug(f"⚠️ Order not found in CoW API: {order_uid}")
                return None
            else:
                logger.warning(f"⚠️ CoW API error {response.status_code}: {response.text}")
                return None
                
        except requests.exceptions.RequestException as e:
            logger.warning(f"⚠️ CoW API request failed: {e}")
            return None
        except Exception as e:
            logger.warning(f"⚠️ Failed to parse CoW API response: {e}")
            return None

    def find_shapeshift_affiliates_in_csv(self, csv_file: str = None):
        """Find all ShapeShift affiliate transactions in the enriched CSV."""
        if csv_file is None:
            csv_file = os.path.join(self.csv_dir, "cowswap_transactions_enriched.csv")
        
        if not os.path.exists(csv_file):
            logger.error(f"❌ Enriched CSV file not found: {csv_file}")
            logger.info("💡 Run --enrich-csv first to get affiliate data from CoW API")
            return
        
        logger.info(f"🔍 Searching for ShapeShift affiliate transactions in: {csv_file}")
        ...
        # Find ShapeShift affiliates
        shapeshift_txs = []
        for tx in transactions:
            if (tx.get('cow_api_affiliate', '').lower() == 'shapeshift' or
                'shapeshift' in tx.get('cow_api_app_code', '').lower() or
                'shapeshift' in tx.get('cow_api_app_data', '').lower() or
                tx.get('affiliate_address', '').lower() == 'shapeshift'):
                shapeshift_txs.append(tx)
        
        if shapeshift_txs:
            logger.info(f"🎉 Found {len(shapeshift_txs)} ShapeShift affiliate transactions!")
            
            # Display results
            for i, tx in enumerate(shapeshift_txs):
                logger.info(f"\n📊 ShapeShift Affiliate Transaction {i+1}:")
                logger.info(f"   Hash: {tx['tx_hash']}")
                logger.info(f"   Block: {tx['block_number']}")
                logger.info(f"   Date: {tx.get('date', tx.get('block_time', 'N/A'))}")
                logger.info(f"   Order ID: {tx['order_uid']}")
                logger.info(f"   CoW API Affiliate: {tx.get('cow_api_affiliate', 'N/A')}")
                logger.info(f"   App Code: {tx.get('cow_api_app_code', 'N/A')}")
                logger.info(f"   App Data: {tx.get('cow_api_app_data', 'N/A')}")
        else:
            logger.info("❌ No ShapeShift affiliate transactions found")
            logger.info("💡 This could mean:")
            logger.info("   - ShapeShift doesn't use CoW Swap as an affiliate")
            logger.info("   - The affiliate data is stored differently")
            logger.info("   - Need to scan different time periods")
        
        return shapeshift_txs
    ...
    def get_csv_stats(self):
        ...
        try:
            ...
            print(
                f"   Date range: {df['block_time'].min()} to {df['block_time'].max()}"
            )
            ...
            if len(df) > 0:
                ...
                for _, row in recent.iterrows():
                    print(
                        f"   {row['chain']}: {row['sell_token_name']} → {row['buy_token_name']} - {row['tx_hash'][:10]}..."
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="CSV-based CoW Swap Affiliate Fee Listener"
    )
    parser.add_argument(
        "--blocks", type=int, default=1000, help="Number of blocks to scan"
    )
    parser.add_argument(
        "--enrich-csv", action='store_true', help='Enrich existing CSV with CoW Swap API affiliate data'
    )
    parser.add_argument(
        "--find-affiliates", action='store_true', help='Find ShapeShift affiliate transactions in enriched CSV'
    )
    parser.add_argument(
        "--csv-file", type=str, help='Specific CSV file to process'
    )
    ...
    try:
        ...
        if args.enrich_csv:
            logger.info("🔍 Enriching CSV with CoW Swap API affiliate data...")
            listener.enrich_csv_with_affiliate_data(args.csv_file)
        elif args.find_affiliates:
            logger.info("🔍 Finding ShapeShift affiliate transactions...")
            listener.find_shapeshift_affiliates_in_csv(args.csv_file)
        else:

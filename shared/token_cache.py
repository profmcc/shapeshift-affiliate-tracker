...
def get_token_info(address: str) -> Optional[Dict]:
    ...
    with _DB_LOCK:
        conn = _get_conn()
        cur = conn.execute(
            "SELECT symbol, name, decimals, price FROM tokens WHERE address = ?",
            (address,),
        )
        row = cur.fetchone()
        if row:
        ...
        try:
            ...
            # Insert into cache
            conn.execute(
                'INSERT OR REPLACE INTO tokens (address, symbol, name, decimals, price, updated_at) VALUES (?, ?, ?, ?, ?, strftime("%s","now"))',

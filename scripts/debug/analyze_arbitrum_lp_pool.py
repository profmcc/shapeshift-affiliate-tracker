...
# Configuration
DB_PATH = (
    "arbitrum_weth_fox_events.db"  # You will need to create this DB with event data
)
...
def get_token_price(token_address, block_number=None):
    ...
    try:
        ...
        token_map = {WETH_ADDRESS: "ethereum", FOX_ADDRESS: "shapeshift-fox-token"}
...
def create_visualizations():
    ...
    if result:
        ...
        weth_price = get_token_price(WETH_ADDRESS)
        fox_price = get_token_price(FOX_ADDRESS)
        current_total_usd = (pool_info["weth_reserve"] / 1e18) * weth_price + (
            pool_info["fox_reserve"] / 1e18
        ) * fox_price
        total_removed_usd = 0
    ...
    if not burns_df.empty:
        ...
        ax2.axhline(
            ...
            linestyle="--",
            alpha=0.7,
            label=f"Total: ${total_removed_usd:,.0f}",
        )

...
def test_database_setup():
    ...
    try:
        ...
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='viewblock_thorchain_fees'"
        )
        ...
        # Check indexes
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='viewblock_thorchain_fees'"
        )
...
def test_url_accessibility():
    ...
    logger.info(
        "  This is expected behavior. The scraper will use Selenium to bypass this."
    )
    return True

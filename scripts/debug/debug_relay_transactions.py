            ...
            try:
                ...
                for tx in block["transactions"]:
                    ...
                    if to_addr == relay_contract:
                        ...
                        if not transfers:
                            print(
                                "   ❌ NO TRANSFERS FOUND - This explains the $0 volume!"
                            )
                            ...
                            for i, transfer in enumerate(transfers):
                                ...
                                if token_info:
                                    amount_formatted = transfer["amount"] / (
                                        10 ** token_info["decimals"]
                                    )
                                    usd_value = amount_formatted * token_info["price"]
                                    print(
                                        f"     ✅ Recognized: {amount_formatted:.6f} {token_info['symbol']} = ${usd_value:.2f}"
                                    )

                                    if i == 0:  # First transfer
                                        total_recognized_value = usd_value
                                        print(
                                            f"     🎯 This is the FIRST transfer - would be used for volume"
                                        )
                                else:
                                    print(
                                        f"     ❌ Unknown token - not in our dictionary"
                                    )
                            ...
                            if total_recognized_value > 0:
                                print(
                                    f"   Initial transfer value: ${total_recognized_value:.2f}"
                                )

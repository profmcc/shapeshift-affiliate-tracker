        ...
        try:
                        ...
                        affiliate_swaps.append(
                            {
                                ...
                                "from_amount": float(
                                    swap.get("from", {}).get("amount", 0)
                                )
                                / 1e8,
                                "to_amount": float(swap.get("to", {}).get("amount", 0))
                                / 1e8,
                                "affiliate_fee": float(
                                    swap.get("affiliateFee", {}).get("amount", 0)
                                )
                                / 1e8,
                                "affiliate_fee_asset": swap.get("affiliateFee", {}).get(
                                    "asset", ""
                                ),
            ...
            if affiliate_swaps:
                ...
                for i, swap in enumerate(affiliate_swaps[:3]):
                    print(
                        f"  {i+1}. {swap['from_asset']} -> {swap['to_asset']}, Fee: {swap['affiliate_fee']} {swap['affiliate_fee_asset']}"
                    )
...
def test_thorchain_viewblock_api():
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print(
                f"✅ ViewBlock API response: {len(data) if isinstance(data, list) else 'object'}"
            )
            return data
        else:

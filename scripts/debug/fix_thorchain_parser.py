...
def fetch_thorchain_swaps(limit: int = 1000, offset: int = 0) -> List[Dict]:
    ...
    try:
        ...
        params = {"limit": limit, "offset": offset, "interval": "1d"}  # Daily intervals
...
def fetch_thorchain_actions_with_affiliate(limit: int = 1000) -> List[Dict]:
    ...
    try:
        ...
        params = {"limit": limit, "offset": 0, "type": "swap"}  # Only swap actions
        ...
        logger.info(
            f"Found {len(affiliate_actions)} actions with affiliate data out of {len(actions)} total"
        )
...
def parse_thorchain_action_for_affiliate(action: Dict) -> Optional[Dict]:
    ...
    try:
        ...
        date_str = (
            datetime.fromtimestamp(date_timestamp).isoformat() if date_timestamp else ""
        )
        ...
        affiliate_fee_amount = (
            float(affiliate_fee_raw.get("amount", 0)) / 1e8 if affiliate_fee_raw else 0
        )
        affiliate_fee_asset = (
            affiliate_fee_raw.get("asset", "") if affiliate_fee_raw else ""
        )
        ...
        # Create swap path
        from_asset_name = (
            from_asset.split(".")[-1] if "." in from_asset else from_asset.split("-")[0]
        )
        to_asset_name = (
            to_asset.split(".")[-1] if "." in to_asset else to_asset.split("-")[0]
        )
...
def get_database_stats():
    ...
    try:
        ...
        cursor.execute(
            "SELECT MIN(block_timestamp), MAX(block_timestamp) FROM thorchain_swaps"
        )
        ...
        if min_time and max_time:
            print(
                f"Time range: {datetime.fromtimestamp(min_time)} - {datetime.fromtimestamp(max_time)}"
            )

        # Show sample data
        cursor.execute(
            "SELECT tx_id, affiliate_address, swap_path, affiliate_fee_usd FROM thorchain_swaps LIMIT 5"
        )

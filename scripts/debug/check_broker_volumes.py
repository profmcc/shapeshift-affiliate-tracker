...
async def check_broker_volume(broker_url: str):
    ...
    try:
        ...
        print(
            f"      Transaction ID Range: {min(transaction_ids) if transaction_ids else 'N/A'} to {max(transaction_ids) if transaction_ids else 'N/A'}"
        )
...
async def main():
    ...
    try:
        ...
        for result in results:
            print(
                f"   {result['broker']}: ${result['volume']:,.2f} ({result['transactions']:,} transactions)"
            )

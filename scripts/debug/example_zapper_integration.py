...
class ZapperPortfolioAnalyzer:
    ...
    async def scrape_and_analyze(self, url: str) -> Optional[Dict]:
        ...
        analysis_file = (
            self.output_dir
            / f"portfolio_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    async def analyze_portfolio(self, csv_file: str) -> Dict:
        ...
        top_holdings = df.nlargest(10, "Value_Numeric")[
            ["Token", "Value", "Value_Numeric"]
        ]

        # Token distribution
        token_distribution = (
            df.groupby("Token")["Value_Numeric"].sum().sort_values(ascending=False)
        )
        ...
        analysis = {
            ...
            "portfolio_summary": {
                ...
                "average_token_value": total_value / token_count
                if token_count > 0
                else 0,
            },
    ...
    def save_to_database_format(self, csv_file: str, db_format: str = "sqlite") -> str:
        ...
        if db_format == "sqlite":
            db_file = (
                self.output_dir
                / f"zapper_portfolio_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
            )
            import sqlite3
            ...
            parquet_file = (
                self.output_dir
                / f"zapper_portfolio_{datetime.now().strftime('%Y%m%d_%H%M%S')}.parquet"
            )

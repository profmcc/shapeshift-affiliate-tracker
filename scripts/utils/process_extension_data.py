...
def process_extension_data(
    json_file: str, db_path: str = "viewblock_extension_data.db"
):
    ...
    for tx in data:
        try:
            ...
            logger.error(
                f"Error processing transaction {tx.get('tx_hash', 'unknown')}: {e}"
            )
            skipped_count += 1
            continue
...
def get_database_stats(db_path: str = "viewblock_extension_data.db"):
    ...
    cursor.execute(
        "SELECT MIN(timestamp), MAX(timestamp) FROM viewblock_extension_data WHERE timestamp IS NOT NULL"
    )
    time_range = cursor.fetchone()

    cursor.execute(
        "SELECT COUNT(DISTINCT from_address) FROM viewblock_extension_data WHERE from_address IS NOT NULL"
    )
    unique_addresses = cursor.fetchone()[0]

    cursor.execute(
        "SELECT COUNT(DISTINCT from_asset) FROM viewblock_extension_data WHERE from_asset IS NOT NULL"
    )
    unique_assets = cursor.fetchone()[0]

    cursor.execute(
        "SELECT from_asset, COUNT(*) FROM viewblock_extension_data WHERE from_asset IS NOT NULL GROUP BY from_asset ORDER BY COUNT(*) DESC LIMIT 5"
    )
...
def export_to_csv(
    db_path: str = "viewblock_extension_data.db", output_file: str = None
):
...
def main():
    ...
    parser.add_argument(
        "--db", default="viewblock_extension_data.db", help="Database file path"
    )

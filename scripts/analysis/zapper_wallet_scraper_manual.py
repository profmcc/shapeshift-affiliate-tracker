...
class ManualZapperScraper:
        ...
        try:
            ...
            for method in extraction_methods:
                try:
                    ...
                    if data:
                        logger.info(
                            f"✅ Successfully extracted {len(data)} tokens using {method.__name__}"
                        )
                        return data
                except Exception as e:
            ...
            try:
                ...
                for container in containers:
                    try:
                        ...
                        token_elem = await container.query_selector(
                            '[class*="name"], [class*="symbol"], [class*="token-name"]'
                        )
                        price_elem = await container.query_selector('[class*="price"]')
                        balance_elem = await container.query_selector(
                            '[class*="balance"], [class*="amount"]'
                        )
                        value_elem = await container.query_selector(
                            '[class*="value"], [class*="total"]'
                        )

                        if token_elem:
                            token = await self._extract_text(token_elem)
                            price = (
                                await self._extract_text(price_elem)
                                if price_elem
                                else ""
                            )
                            balance = (
                                await self._extract_text(balance_elem)
                                if balance_elem
                                else ""
                            )
                            value = (
                                await self._extract_text(value_elem)
                                if value_elem
                                else ""
                            )
                ...
                for list_elem in lists:
                    ...
                    for item in items:
                        try:
                            ...
                            token_elem = await item.query_selector(
                                '[class*="name"], [class*="symbol"]'
                            )
                            price_elem = await item.query_selector('[class*="price"]')
                            balance_elem = await item.query_selector(
                                '[class*="balance"]'
                            )
    ...
    def save_data(
        self, data: List[Dict[str, str]], output_dir: str = "data"
    ) -> Tuple[str, str]:
    ...
    async def scrape_wallet_data(
        self, url: str, output_dir: str = "data"

...
class AutomatedZapperTracker:
    ...
    def __init__(
        self, data_dir: str = "data", history_file: str = "portfolio_history.json"
    ):
    ...
    async def capture_snapshot(self, url: str) -> Optional[Dict]:
        ...
        df["Value_Numeric"] = pd.to_numeric(
            df["Value"].str.replace("$", "").str.replace(",", ""), errors="coerce"
        )
        ...
        logger.info(
            f"✅ Snapshot captured: {snapshot['total_value_formatted']} ({token_count} tokens)"
        )
        return snapshot
    ...
    def analyze_changes(self, days_back: int = 7) -> Dict:
        ...
        value_change_pct = (
            (value_change / oldest["total_value"]) * 100
            if oldest["total_value"] > 0
            else 0
        )
        ...
        removed_tokens = [
            t for token, t in old_tokens.items() if token not in new_tokens
        ]
    ...
    def generate_change_report(self, analysis: Dict) -> str:
        ...
        if analysis["added_token_details"]:
            report_lines.extend(["", "➕ ADDED TOKENS", "-" * 30])
        ...
        if analysis["removed_token_details"]:
            report_lines.extend(["", "➖ REMOVED TOKENS", "-" * 30])

...
class THORChainListener:
    def __init__(self, db_path: str = "databases/thorchain_transactions.db"):
        ...
        self.shapeshift_affiliate_ids = [
            "ss",
            "thor1z8s0yk6q86nqwsc2gagv4n9yt9c0hk9qtszt0p",
        ]
        self.init_database()
    ...
    def process_thorchain_action(self, action: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            try:
                timestamp = (
                    int(datetime.fromisoformat(date.replace("Z", "+00:00")).timestamp())
                    if date
                    else 0
                )
            except ValueError:
            ...
            if inputs:
                from_amount = (
                    float(inputs[0].get("coins", [{}])[0].get("amount", 0)) / 1e8
                )

            if outputs:
                to_amount = (
                    float(outputs[0].get("coins", [{}])[0].get("amount", 0)) / 1e8
                )
            ...
            affiliate_fee_amount = (
                from_amount * (affiliate_fee_basis_points / 10000.0)
            ...
            transaction = {
                ...
                "is_streaming_swap": swap_data.get("streamingSwap", {}).get(
                    "quantity", 0
                )
                > 1,
            ...
            logger.info(
                f"   ✅ Found ShapeShift affiliate swap: {tx_id} ({from_asset} -> {to_asset})"
            )
            return transaction
            ...
            try:
                ...
                """,
                    (
                        ...
                        tx["affiliate_fee_basis_points"],
                        tx["affiliate_fee_amount"],
                        tx["affiliate_fee_usd"],
                        tx["from_asset"],
                        tx["to_asset"],
                        tx["from_amount"],
                        tx["to_amount"],
    ...
    def run_listener(self, limit: int = 100):
        ...
        logger.info(
            f"✅ THORChain listener completed! Found {len(affiliate_transactions)} affiliate transactions"
        )
...
def main():
    ...
    parser.add_argument(
        "--limit", type=int, default=100, help="Number of actions to fetch"
    )

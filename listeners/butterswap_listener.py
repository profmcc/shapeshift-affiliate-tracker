...
class ButterSwapListener:
    ...
    def __init__(self, csv_file: str = "csv_data/butterswap_transactions.csv"):
        ...
        logger.info(
            f"Primary focus: Base chain (0x35339070f178dC4119732982C23F5a8d88D3f8a3)"
        )
    ...
    def _init_block_trackers(self):
        ...
        for chain_id in self.web3_connections.keys():
            tracker_file = (
                f"csv_data/block_tracking/butterswap_block_tracker_{chain_id}.csv"
            )
    ...
    def scan_chain(
        ...
        logger.info(
            f"🔍 Scanning chain {chain_id} from block {start_block} to {end_block}"
        )
        ...
        while current_block <= end_block:
            ...
            try:
                ...
                logger.error(
                    f"Error processing batch {current_block}-{batch_end} on chain {chain_id}: {e}"
                )
    ...
    def _process_block_batch(self, chain_id: int, start_block: int, end_block: int):
        ...
        for block_num in range(start_block, end_block + 1):
            try:
                ...
                logger.error(
                    f"Error processing block {block_num} on chain {chain_id}: {e}"
                )
                continue
    ...
    def _process_transaction(self, chain_id: int, block_number: int, transaction):
        ...
        if transaction.to and transaction.to.lower() in [
            addr.lower() for addr in shapeshift_addresses
        ]:
        ...
        try:
            receipt = self.web3_connections[chain_id].eth.get_transaction_receipt(
                transaction.hash
            )

            for log in receipt.logs:
                if self._is_affiliate_fee_log(log, shapeshift_addresses):
                    self._handle_affiliate_fee_log(
                        chain_id, block_number, transaction, log
                    )

        except TransactionNotFound:
            logger.warning(
                f"Transaction {transaction.hash.hex()} not found on chain {chain_id}"
            )
        except Exception as e:
            logger.error(
                f"Error processing transaction {transaction.hash.hex()} on chain {chain_id}: {e}"
            )
    ...
    def _handle_affiliate_transaction(
        self, chain_id: int, block_number: int, transaction
    ):
        """
    ...
    def _handle_affiliate_fee_log(
        self, chain_id: int, block_number: int, transaction, log
    ):
        """
    ...
    def _store_transaction_data(self, **kwargs):
        ...
        logger.info(
            f"💾 Stored transaction data: {kwargs.get('transaction_hash', 'Unknown')}"
        )
...
def main():
    ...
    try:
        ...
        parser = argparse.ArgumentParser(
            description="ButterSwap Affiliate Fee Listener"
        )

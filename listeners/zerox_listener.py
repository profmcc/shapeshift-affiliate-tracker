...
class ZeroXListener:
    ...
    def fetch_zerox_events(
        ...
        try:
            ...
            logger.info(
                f"🔍 Scanning {chain_config['name']} 0x Protocol blocks {start_block} to {latest_block}"
            )
    ...
    def parse_zerox_event(self, log, w3: Web3, chain_config: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            logger.info(
                f"   ✅ Found ShapeShift 0x {event_type}: {log['transactionHash'].hex()} (${affiliate_amount/1e18:.4f} fee)"
            )
            return event_data
    ...
    def get_database_stats(self):
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM zerox_transactions WHERE event_type = 'fill'"
        )
